...
def draft_to_saved_strategy(request: BuilderBacktestRequest) -> SavedStrategy:
...
def _strategy_description(strategy: SavedStrategy) -> str:
    if strategy.description.strip():
        return strategy.description.strip()
    labels = {item["id"]: item["label"] for item in list_indicators(builder_only=True)}
    return format_condition_preview(
        [condition.model_dump() for condition in strategy.conditions],
        labels,
    )


def _compile_strategy_masks(data: pd.DataFrame, strategy: SavedStrategy):
...
def combine_builder_buy_masks(
    ...
    p_buy, p_sell = _compile_strategy_masks(data, primary)
    s_buy, _ = _compile_strategy_masks(data, secondary)
    ...
    p_desc = _strategy_description(primary)
    s_desc = _strategy_description(secondary)
...
def builder_signal_callable(strategy: SavedStrategy) -> Callable:
...
def backtest_builder_signal_combinations(
    ...
    signal_a = builder_signal_callable(primary)
    signal_b = builder_signal_callable(secondary)
